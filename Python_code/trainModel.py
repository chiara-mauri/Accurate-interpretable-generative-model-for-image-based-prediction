...
def trainModel(trainImages,trainBasisFun,nLatentVars,costTol=1e-5, nItMin=10, nItMax=500):
    ...
    for nIt in np.arange(nItMax):
        ...
        dimBlock = 20000
        nBlocksTot = np.ceil(nVoxels/dimBlock)
        V = np.zeros([nVoxels,nLatentVars])
        for n_block in np.arange(nBlocksTot):
            voxelsBlock = np.arange(n_block*dimBlock,np.amin([(n_block+1)*dimBlock,nVoxels]),dtype=int)
            D_partial=np.dot(np.transpose(trainImages[:,voxelsBlock])-np.dot(W[voxelsBlock,:],np.transpose(trainBasisFun)),muZ)
            V[voxelsBlock,:] = np.transpose(np.linalg.solve(np.transpose(A),np.transpose(D_partial)))
